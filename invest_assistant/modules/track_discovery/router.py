...
def create_thesis(
    ...
    track = service.create_track(db, TrackCreate(name=payload.title, status=payload.status))
    return service.create_thesis(db, track["id"], payload, user.id)
...
def list_indicators(thesis_id: int, db: Session = Depends(get_db)) -> list:
    ...
    if thesis is None:
        raise HTTPException(status_code=404, detail="track thesis not found")
    return service.list_indicators(db, thesis.track_id)
...
def add_indicator(thesis_id: int, payload: TrackValidationIndicatorCreate, db: Session = Depends(get_db)):
    ...
    if thesis is None:
        raise HTTPException(status_code=404, detail="track thesis not found")
    return service.add_indicator(db, thesis.track_id, payload, thesis_id)
...
def list_evidence(thesis_id: int, db: Session = Depends(get_db)) -> list:
    ...
    if thesis is None:
        raise HTTPException(status_code=404, detail="track thesis not found")
    return service.list_evidence(db, thesis.track_id)
...
def add_evidence(thesis_id: int, payload: TrackEvidenceCreate, db: Session = Depends(get_db)):
    ...
    if thesis is None:
        raise HTTPException(status_code=404, detail="track thesis not found")
    return service.add_evidence(db, thesis.track_id, payload, thesis_id)
...
def list_related_stocks(thesis_id: int, db: Session = Depends(get_db)) -> list:
    ...
    if thesis is None:
        raise HTTPException(status_code=404, detail="track thesis not found")
    return service.list_related_stocks(db, thesis.track_id)
...
def add_related_stock(thesis_id: int, payload: TrackRelatedStockCreate, db: Session = Depends(get_db)):
    ...
    if thesis is None:
        raise HTTPException(status_code=404, detail="track thesis not found")
    return service.add_related_stock(db, thesis.track_id, payload, thesis_id)
