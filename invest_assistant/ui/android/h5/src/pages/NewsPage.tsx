...
export function NewsPage() {
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam, signal }) => {
      ...
      const page = await mobileApi.news({ limit: 30, offset: pageParam, ...newsQueryForTab(tab) }, signal);
      if (serial !== requestSerial.current) throw new DOMException("stale request", "AbortError");
      return page;
    },
    getNextPageParam: (last) => last.has_more ? last.offset + last.limit : undefined
  });
  const swipe = useSwipeTabs(tabs, tab, (next) => {
