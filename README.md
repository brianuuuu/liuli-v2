...
- `market_radar`：市场雷达、数据源、快讯、标签、候选标签、热度、关系图、任务注册。
...
- 控制台基础运维闭环：任务中心、股票基础库、系统配置、报告库、公告财报库、标签库、候选标签；任务中心采用紧凑卡片模式，运行记录在右侧抽屉中使用事件流列表，保留运行、配置、日志与详情操作。
- 市场雷达业务工作台：总览、热度榜、数据源、快讯、标签、候选标签、标的关系图；数据源页用于底层 `source_item` 管理，快讯页以时间轴展示新闻信息并可直接同步财联社最新快讯。
