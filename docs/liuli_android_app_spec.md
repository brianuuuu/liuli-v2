...
- H5 构建产物通过现有 `5173/mobile/` 提供，服务器不增加常驻前端进程。
...
5. `invest_assistant/ui/web/public/mobile/` 只是被 Git 忽略的部署产物目录，不是共享源码。
...
- 仅允许当前服务器 `/mobile/` 与 `/api/`；外部链接交给系统浏览器。
...
```text
http://115.29.176.240:5173/mobile/#/news
```
...
开发端口为 `5174`，只用于本地热更新，不作为服务器常驻进程。
默认将 `/api` 代理到公网服务器，便于直接使用真实数据调试；需要改用本机 `8000`
时，可在启动前设置 `VITE_API_PROXY_TARGET`。该变量只影响开发代理，不进入生产包。
...
构建先输出到 H5 自身 `dist`，同步脚本再复制到被 Git 忽略的 `invest_assistant/ui/web/public/mobile/`。

根目录 `start.bat` 和 `start.sh` 在启动现有 5173 Web 进程前完成 H5 构建和同步，因此线上不增加 Node 常驻进程。
