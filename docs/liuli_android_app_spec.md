...
底栏内容高 `56dp`，额外消费系统手势导航安全区。底栏 Surface 必须覆盖包括手势安全区在内的完整底部区域，浅色为纯白、深色为纯黑，不得露出 Scaffold 背景。图标和文字均由同一个 Compose 组件绘制，位置不受 H5 滚动和业务页面影响。
...
- H5 同时向原生层发布当前路由是否可内部返回；Android 系统返回键与 H5 顶部返回键走同一路由回退。普通详情优先回来源页，直接打开详情时回所属根页，只有五个根页允许系统退出 App。
...
- H5 顶部从原生状态栏安全区下方开始。
- 页面顶部区域与 `SecondaryNavigation` 均固定为 `36px`，不保留额外顶部内边距。
- 不使用外边框或单项圆角背景。
- 选中项为蓝色文字、较高字重和 `2px` 短下划线。
- 浅色状态栏安全区和导航固定为连续纯白背景，深色固定为连续纯黑背景。
- 所有模块正文从同一垂直坐标开始。
...
- 笔记和资讯的列表正文、编辑正文及详情正文统一为 `14px`；标签、来源和时间使用 `11–12px`。
- 点击控件目标高度通常不低于 `44px`。
- H5 不重复显示无价值的原生模块标题栏。
- 根页面、详情、卡片和编辑器必须限制在视口宽度内；长链接、长标签和连续字符必须换行，不允许产生页面级横向滚动。

### 5.3 根页面横向手势

- 看板、资讯、笔记和待办的内容区统一使用 `HorizontalTabPager`，二级导航保持固定。
- 内容跟随手指移动；水平位移超过 `60px` 且明显大于垂直位移时，以约 `220ms` 动画吸附到相邻页，否则回弹。
- 点击二级导航也必须经过同一 Pager 吸附流程，不允许直接替换内容；动画距离以内容容器实际宽度计算。
- Pager 只挂载当前页及相邻页，页面数据继续由 TanStack Query 缓存，禁止一次触发全部资讯筛选请求。
- 保存并恢复每个 Tab 的纵向滚动位置。
- 输入框、编辑弹层和已发生横向溢出的分组导航不参与页面切换；分组导航未溢出时允许滑动换组。
- “我的”只有一个二级项，不执行滑动切换；普通详情页不接入根页面手势。
...
- 今日：现有 `/api/console/workbench-today` 只读返回的全部主要指数、组合表现，以及重要资讯、未读预警、最新报告和最近笔记；手机端不提供行情刷新。
- 市场：信息量、标签量和 `type=all` 的 24 小时热度排行。
- 赛道：升温赛道、重点赛道、热度排行和最新材料。
- 标的：标的池、重点标的和评分排行。
- 组合：对齐 Web 组合看板，支持所有/单组合切换，显示总市值、持仓市值、现金余额、年度盈亏、今日表现、标的市值占比和 180 天组合市值曲线。后端 `day_pct` 已是百分数，手机端不得再次乘以 100。

### 6.2 笔记

- 二级导航直接显示“全部”和服务端笔记分组，可点击、左右滑动切换；分组较多时导航本身可横向滚动。
- “编辑分组”固定在二级导航最右侧，点击打开管理面板，不参与筛选、选中状态和 Pager 顺序。
- 支持编辑分组、新增分组和归档分组。
- 列表采用 Flomo 式短文本卡片，不显示派生标题和 `note_type`。
- 新增和编辑均为纯文本，不提供 Markdown、图片、语音和本地草稿。
- 编辑 textarea 使用软换行并禁止内部横向滚动；长 URL 和连续字符不得产生页面级横向滚动条。
- 标签字号与正文接近，不形成夸张层级。
- 提交失败时保留当前输入面板内容并允许重试。
- 详情支持归档和删除；两项操作均须二次确认，调用既有软状态变更接口，成功后返回笔记列表并刷新缓存。
...
资讯采用日期吸顶、左侧时间轨道和节点的 7×24 时间线。二级 Tab 支持点击和左右滑动；分页每次读取 30 条；切换筛选时通过 `AbortSignal` 取消旧请求，旧结果不得覆盖新筛选结果；按 ID 去重并保存 TanStack Query 缓存。模块路由继续使用 `/news`。
...
待办根路由为 `/tasks`，默认显示预警。预警内部使用“全部｜未读｜已处理”紧凑筛选；现有 `/api/alerts/events` 没有状态参数，因此按现有分页加载后在客户端筛选。若当前已加载页没有目标状态但服务端仍有后续页，按单请求顺序继续渐进加载，直到找到结果或到达末页，禁止显示假空态或发起并行重复请求。预警详情路由为 `/tasks/alerts/{id}`。

详情支持现有标记已读和已处理接口，不增加处理备注字段。

“AI 推荐词”对齐 Web 审核能力：支持搜索、待审核/已通过/已拒绝筛选、分页、新增、通过、拒绝和恢复。通过时支持设置最终标签词，并绑定市场热词、赛道或已有标的；标的必须从股票主数据搜索结果中选择。

### 6.5 我的

二级导航显示单项“设置”，保持与其他模块相同的 `36px` 顶部结构。
...
| 今日主要指数（只读例外） | `/api/console/workbench-today` |
| 市场看板 | `/api/market-radar/overview`、`/api/market-radar/rankings` |
| 资讯 | `/api/market-radar/source-items`、`/api/market-radar/source-items/{id}` |
...
| 待办-AI 推荐词 | `/api/market-radar/ai-tag-suggestions`、`/approve`、`/reject`、`/restore`，以及热词、赛道和股票搜索接口 |
| 报告 | `/api/reports`、`/api/reports/{id}`、`/content` |

除只读 GET `/api/console/workbench-today` 外，禁止调用其他 `/api/console/*`；禁止调用行情刷新接口，也禁止为手机端新增聚合接口、筛选参数或数据库字段。
...
- 资讯筛选参数、预警本地筛选和 AI 推荐词审核请求。
- API Query、Token Header 和 401。
- JS Bridge 缺失时的浏览器兼容。
- 登录、资讯根页统一二级导航、待办根页与详情路由、报告隐藏底栏。
- 顶部返回、Android 系统返回、直接打开详情时的所属根页兜底。
- 笔记归档/删除和组合百分比口径。
...
- 二级导航顶部、总高和正文起点完全一致。
- 页面 `scrollWidth` 不得大于视口宽度；长链接、长标签和连续字符不得撑宽笔记页面。
- 笔记和资讯的主文字计算字号均为 `14px`。
- 顶部、正文、空态和加载态均可触发根页面左右切换，纵向滚动和输入区域不得误触发。
- 原生底栏只做静态编译和桥接模型验证；最终真机安装时确认它不随 H5 滚动、不与内容重叠，且手势安全区与底栏保持同一纯白或纯黑背景。
- 普通详情保留底栏，报告阅读隐藏底栏。
- 笔记纯文本、资讯时间线、待办筛选和 admin 账号展示符合本规格。
