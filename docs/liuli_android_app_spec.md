...
构建结果只保留在 H5 自身 `dist`，不复制到桌面 Web。Linux 5174 必须返回
带哈希的 `/assets/` 生产资源，响应中不得包含 `/@vite/client` 或 React Refresh。
...
- 资讯筛选参数、预警本地筛选、AI 推荐词详情审核和无确认顺序批量拒绝。
