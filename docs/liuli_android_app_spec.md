...
- 笔记和新闻的列表正文、编辑正文及详情正文统一为 `14px`；标签、来源和时间使用 `11–12px`。
- 点击控件目标高度通常不低于 `44px`。
- H5 不重复显示无价值的原生模块标题栏。
...
- 看板、笔记、新闻和预警的内容区统一使用 `HorizontalTabPager`，二级导航保持固定。
- 内容跟随手指移动；水平位移超过 `60px` 且明显大于垂直位移时，以约 `220ms` 动画吸附到相邻页，否则回弹。
- 点击二级导航也必须经过同一 Pager 吸附流程，不允许直接替换内容；动画距离以内容容器实际宽度计算。
- Pager 只挂载当前页及相邻页，页面数据继续由 TanStack Query 缓存，禁止一次触发全部新闻筛选请求。
- 保存并恢复每个 Tab 的纵向滚动位置。
...
- 今日：现有 `/api/console/workbench-today` 只读返回的全部主要指数，以及重要新闻、未读预警、最新报告和最近笔记；手机端不提供行情刷新。
...
现有 `/api/alerts/events` 没有状态参数，因此按现有分页加载后在客户端筛选。二级 Tab 支持点击和左右滑动；若当前已加载页没有目标状态但服务端仍有后续页，按单请求顺序继续渐进加载，直到找到结果或到达末页，禁止显示假空态或发起并行重复请求。
