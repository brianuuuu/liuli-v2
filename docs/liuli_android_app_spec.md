...
- Android 复用 Web 的 `invest_assistant/ui/web/public/favicon.svg`，不另行设计移动端 Logo。
...
- 接受固定 API 地址、手动侧载 APK、个人签名和少量技术化设置，不需要应用商店、账号注册和新手初始化向导。
...
首版只服务于用户本人，默认连接一个固定公网 HTTPS 服务，安装到一台当前主力 Android 手机。设计与工程决策优先满足该设备上的操作效率、数据不丢失和阅读体验，不为潜在公众用户预留通用化能力。
...
- Android 12 以下旧设备、平板、折叠屏、车机、电视和大范围厂商兼容。
...
展示当前账号、主题、构建时配置的 API 地址、本地草稿、修改密码、退出登录和应用版本。主题提供“浅色 / 深色 / 跟随系统”三个明确选项。

API 地址首版只读，通过构建配置注入，不在 App 内任意修改。
...
- 单 Activity。
- `minSdk 31`（Android 12），不为更早系统编写兼容分支。
- `compileSdk / targetSdk` 使用创建工程时 Android Studio 已支持的最新稳定 SDK，并在工程 README 记录实际值。
- Java 17。
- 包名 `com.liuli.app`。
- 应用名“琉璃”。
- 品牌图形唯一来源为 `invest_assistant/ui/web/public/favicon.svg`。
- 最终适配基准为用户当前主力手机的分辨率、系统版本和系统字体设置；模拟器只用于快速开发，不建立公众设备矩阵。

具体依赖版本在创建工程时按当时稳定且互相兼容的版本锁定到 Gradle Version Catalog，不复制旧工程版本号。工程 README 保存 AGP、Kotlin、Compose BOM、Hilt、Room、Retrofit、compileSdk 和 targetSdk 的版本矩阵；后续升级依赖时同步更新矩阵。
...
- Release API 地址由 Gradle BuildConfig 注入，必须使用 HTTPS。
...
- 不保存 Cookie，不启用明文 HTTP，不在日志中输出密码和完整 Token。
- 不做证书锁定、双向 TLS、设备绑定或 Token 二次加密；Token 保存于应用私有 DataStore，满足个人设备使用即可。

### 6.5 构建与安装

- 只保留 `debug` 和 `release` 两种 Build Type，不建设渠道包、公众测试轨道和远程配置。
- `API_BASE_URL` 由本地 Gradle 配置或环境变量注入，构建时去除首尾空格并统一保证以 `/` 结尾；Release 构建缺失或不是 HTTPS 时直接失败。
...
- Manifest 首版只申请 `INTERNET`；不申请通知、定位、通讯录、相机、存储等无关权限。报告通过系统选择器和 `FileProvider` 打开，不申请广泛存储权限。
...
看板、新闻、报告和预警首版只做进程内缓存；无网络时可保留本次进程已经加载的数据，不承诺冷启动离线阅读。
...
Web 当前在 `AppLayout.tsx` 中直接使用 `/favicon.svg`。Android 必须以同一文件作为唯一品牌源：

```text
invest_assistant/ui/web/public/favicon.svg
```

已确认的 Android Launcher Icon 视觉参考图保存于：

```text
docs/assets/android/android-launcher-icon-v1.png
```

该 PNG 用于后续 Android 工程的桌面效果和安全区参考；正式 adaptive icon 仍应从 Web SVG 拆分或确定性生成 foreground/background 资源，不能用单张带底色 PNG 代替所有密度和蒙版资源。
...
- Web Logo 发生变化时，从新的 `favicon.svg` 重新生成 Android 资源，禁止两端分别维护。
...
- 登录页、AppBar、关于页和 Launcher 使用由 Web `favicon.svg` 生成的同一 Logo 资源，且两种主题下不被 tint。
- 在用户主力手机当前字体和显示大小下检查文字、图表、表单和 `48 × 48dp` 高频操作区域，不建立额外设备和无障碍矩阵。
- 静态扫描或架构测试禁止 feature 直接声明原始颜色，原始色值仅存在于设计系统模块。

### 11.5 主力真机验证

- 只在用户当前主力 Android 12+ 手机完成正式验收，模拟器用于开发过程检查。
- 覆盖公网 HTTPS 登录、冷启动、前后台切换、进程回收、断网恢复、主题切换、笔记草稿、报告下载与系统应用打开。
- 覆盖该手机实际使用的浅色/深色、字体大小、显示大小和系统导航方式。
...
11. 登录页、应用栏、关于页和 Launcher 复用 Web `favicon.svg` 的图形，不出现临时文字 Logo 或另一套品牌图形。
...
18. Release APK 在用户当前主力 Android 12+ 手机上完成公网 HTTPS 全流程验收；不要求应用商店、旧设备、平板或多厂商兼容。
