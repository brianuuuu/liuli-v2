...
- v23：补充三类别名维护入口；明确标的别名主入口在股票基础库、辅入口在标的详情；赛道别名主入口在赛道详情；热点词别名主入口在热点词详情、辅入口在标签索引；候选标签审核页提供快捷转别名入口。
...
别名不是 tag 的通用属性，而是标的、赛道、热点词各自的识别属性。
...
```text
stock.stock_name = 官方股票名称，来自 A 股基础库同步或人工修正；
stock_alias.alias = 别名、俗称、曾用名、英文名、简称。
```
...
如果只是补充市场叫法，可在股票基础库维护，也可在标的详情快捷新增，但最终都写入：
...
```text
/console/tags 可以查看热点词标签、别名命中情况、异常重复和停用状态；
但热点词别名的主要编辑入口仍然放在市场雷达 → 热点词详情。
```
...
```text
标的别名归股票基础库，赛道别名归赛道详情，热点词别名归热点词详情；
候选标签审核页提供快捷转别名入口；
source_tag.trigger_text 保留原文命中词，source_tag.tag_id 只记录归一后的正式 tag。
```
...
热点词别名 → /market-radar/hotwords/[tag_id]，/console/tags 仅作辅助治理入口
```
